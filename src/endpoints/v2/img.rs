...
pub async fn get(
    ...
    let query = match params.nsfw {
        Some(nsfw) => sqlx::query_as(
            "select name, provider, nsfw from asset \
                               where nsfw=? and category=? order by random() limit 1",
        )
        .bind(nsfw),
        None => sqlx::query_as(
            "select name, provider, nsfw from asset \
    ...
    let (name, provider, nsfw): (String, String, bool) = query
        .bind(category)
        .fetch_one(&mut conn)
        .await
